...
public class Client extends Frame implements KeyListener{
	...
	private static String serverIp = "192.168.1.44";
	private static int portNum = 9080;
  ...
  public void connectServer(String ip,int port) {
  	...
  	//portNum = 9080
	  	if(ip == serverIp){
		  	...
		  	conStat = true;
		  	tA.append("* * * * C O N N E C T * * * *");
	  	} else {
	  		conStat = false;
	  		insertTextArea("[Alert]Can Not Connect");
	  	}
  	} catch (Exception e){
  		conStat = false;
  	}
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (key == KeyEvent.VK_ENTER) {
      ...
      //System.out.println("ENTER pressed");
     	...
     	if( conStat == false ) {
     		if( s == serverIp ){
     			connectServer(s, portNum);
     		} else {
	   			insertTextArea("[Alert]Is Not Conencted");
	   		}
     	} else {
	      tA.append("나 : " + s + nLine);
	      insertTextArea(s);
